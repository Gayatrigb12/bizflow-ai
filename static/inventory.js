  ...
  for (const p of filtered) {
    ...
    card.innerHTML = `
      ...
      <div style="display:flex;align-items:center;gap:8px;margin-top:8px">
        <div>
          <div class="pc-qty">${p.quantity}</div>
  ...
  grid.querySelectorAll('[data-delete]').forEach((btn) => {
    btn.addEventListener('click', async (e) => {
      ...
      try {
        ...
        console.error(err);
        toast('Failed to delete product', 'error', 6000);
      }
    });
  });
}
...
async function loadAndRender() {
  try {
    ...
    console.error(err);
    toast('Failed to load products', 'error', 6000);
    const grid = document.getElementById('inventoryGrid');
    if (grid) {
      grid.innerHTML = `
        <div class="empty-state">
          <i>⚠️</i>
          <p>Could not load inventory. <button class="send-btn" onclick="loadAndRender()">Retry</button></p>
        </div>
      `;
    }
  }
...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      console.error(err);
      toast('Failed to save product', 'error', 6000);
    }
  });
