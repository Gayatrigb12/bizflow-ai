...
async function showCustomerProfile(customerId) {
  ...
  try {
    ...
    console.error(err);
    toast('Failed to load customer profile', 'error', 6000);
  }
}
...
async function loadAndRender() {
  try {
    ...
    console.error(err);
    toast('Failed to load customers', 'error', 6000);
    const container = document.getElementById('customersList');
    if (container) {
      container.innerHTML = `
        <div class="empty-state">
          <i>⚠️</i>
          <p>Could not load customers. <button class="send-btn" onclick="loadAndRender()">Retry</button></p>
        </div>
      `;
    }
  }
